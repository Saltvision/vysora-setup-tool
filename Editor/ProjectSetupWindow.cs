...
public class ProjectSetupWindow : EditorWindow
{
    ...
    private void OnEnable()
    {
        ...
        Vysora.Logger.Info("Initializing Vysora Setup Window");
    ...
    private void DrawSetupTab()
    {
        ...
        if (!setupConfirmed)
        {
            ...
            if (GUILayout.Button("Start Installation", GUILayout.Height(40)))
            {
                setupConfirmed = true;
                currentSetupStep = 0;
                Vysora.Logger.Info("Setup process initiated");
            }
        }
        else
        {
            ...
            if (GUILayout.Button("Cancel Setup", GUILayout.Height(25)))
            {
                    ...
                    "Yes", "No"))
                {
                    ...
                    githubToken = "";
                    githubPassword = "";
                    Vysora.Logger.Info("Setup process canceled by user");
                }
            }
        }
    }
    ...
    private void CreateSceneObjects()
    {
        try
        {
            ...
            Vysora.Logger.Info("Creating scene objects");
            ...
            Vysora.Logger.Info("Scene objects created successfully!");
            EditorUtility.DisplayDialog("Success", "Scene objects created successfully!", "OK");
        }
        catch (Exception ex)
        {
            Vysora.Logger.Exception(ex, "Create scene objects");
    ...
    {
        Vysora.Logger.Info("Clearing existing scene objects");
        ...
        Vysora.Logger.Info($"Cleared {objectsToDelete.Count} objects from the scene");
    }
    ...
    private PanelSettings CreatePanelSettings()
    {
        ...
        {
            Vysora.Logger.Info("PanelSettings asset already exists, using existing one");
        ...
        Vysora.Logger.Info($"Created PanelSettings asset at {panelSettingsPath}");
    ...
    private void CreateUIDocumentAsset(UIDocument document)
    {
        ...
        if (!File.Exists(uiDocumentPath))
        {
            ...
            Vysora.Logger.Info($"Created UI Document asset at {uiDocumentPath}");
        }
    ...
    private void CheckGitInstallation()
    {
        try
        {
            ...
            if (!string.IsNullOrEmpty(output))
            {
                ...
                gitInstalled = true;
                Vysora.Logger.Info($"Git found at: {gitExecutablePath}");
            }
            else
            {
                gitInstalled = false;
                Vysora.Logger.Warning("Git not found on system PATH");
            }
        }
        catch (Exception ex)
        {
            gitInstalled = false;
            Vysora.Logger.Exception(ex, "Check git installation");
        }
    }
    ...
    private void LoginToGitHub()
    {
        try
        {
                ...
                (!string.IsNullOrEmpty(githubUsername) && !string.IsNullOrEmpty(githubPassword)))
            {
                ...
                {
                    isLoggedIn = true;
                    Vysora.Logger.Info("GitHub login successful");
        ...
        {
            Vysora.Logger.Exception(ex, "GitHub login");
    ...
    private async void DownloadAssets()
    {
        try
        {
            ...
            Vysora.Logger.Info("Assets downloaded successfully");
        ...
        {
            isDownloading = false;
            Vysora.Logger.Exception(ex, "Download assets");
    ...
    private void CloneRepository()
    {
        ...
        if (isPrivateRepo)
        {
            ...
            if (!string.IsNullOrEmpty(githubToken))
            {
                ...
                Vysora.Logger.Info("Using token authentication for GitHub");
            }
            ...
            else if (!string.IsNullOrEmpty(githubUsername) && !string.IsNullOrEmpty(githubPassword))
            {
                ...
                Vysora.Logger.Info("Using username/password authentication for GitHub");
            }
            else
            {
        ...
        else
        {
            ...
            else
            {
                ...
                Vysora.Logger.Info("Using public access for GitHub (no authentication)");
            }
        }
        ...
        {
            Vysora.Logger.Exception(ex, "Failed to prepare temp directory");
            ...
            Vysora.Logger.Info($"Using alternative temp folder: {tempFolder}");
            Directory.CreateDirectory(tempFolder);
        }
        ...
        try
        {
            ...
            catch (Exception ex)
            {
                ...
                Vysora.Logger.Warning($"Could not remove temp folder: {ex.Message}. Manual cleanup may be required.");
            }
    ...
    private void SafeDeleteDirectory(string directory)
    {
        try
        {
            ...
            if (Application.platform == RuntimePlatform.WindowsEditor)
            {
                ...
                {
                    Vysora.Logger.Warning($"Command line deletion failed, falling back to .NET delete for {directory}");
        ...
        {
            Vysora.Logger.Exception(ex, $"Failed to delete directory {directory}");
            throw;
        }
    }
    private void RelocateFiles(string tempFolder)
    {
        try
        {
            Vysora.Logger.Info("Relocating downloaded files to their correct locations");
            ...
            {
                Vysora.Logger.Info($"Moving 3dModel files from {modelsSource} to {modelsDestination}");
                CopyDirectoryContents(modelsSource, modelsDestination);
            }
            else
            {
                Vysora.Logger.Warning("3dModel folder not found in downloaded repository");
            }
            ...
            {
                Vysora.Logger.Info($"Moving Plugins files from {pluginsSource} to {pluginsDestination}");
                CopyDirectoryContents(pluginsSource, pluginsDestination);
            }
            else
            {
                Vysora.Logger.Warning("Plugins folder not found in downloaded repository");
            }
            ...
            {
                Vysora.Logger.Info($"Moving Scripts files from {scriptsSource} to {scriptsDestination}");
                CopyDirectoryContents(scriptsSource, scriptsDestination);
            }
            else
            {
                Vysora.Logger.Warning("Scripts folder not found in downloaded repository");
            }
            ...
            {
                Vysora.Logger.Info($"Moving Materials files from {materialsSource} to {materialsDestination}");
                CopyDirectoryContents(materialsSource, materialsDestination);
            }
            else
            {
                Vysora.Logger.Warning("Materials folder not found in downloaded repository");
            }
            ...
            {
                Vysora.Logger.Info($"Moving Resources files from {resourcesSource} to {resourcesDestination}");
                CopyDirectoryContents(resourcesSource, resourcesDestination);
            }
            else
            {
                Vysora.Logger.Warning("Resources folder not found in downloaded repository");
            }
            ...
            {
                Vysora.Logger.Info($"Moving UI files from {uiSource} to {uiDestination}");
                CopyDirectoryContents(uiSource, uiDestination);
            }
            else
            {
                Vysora.Logger.Warning("UI folder not found in downloaded repository");
            }

            Vysora.Logger.Info("Files relocated successfully");
        }
        catch (Exception ex)
        {
            Vysora.Logger.Exception(ex, "Failed to relocate files");
    ...
    private void EnsureDirectoryExists(string directory)
    {
        if (!Directory.Exists(directory))
        {
            ...
            Vysora.Logger.Info($"Created directory: {directory}");
        }
    }
    ...
    private void CopyDirectoryContents(string sourceDir, string targetDir)
    {
        ...
        foreach (string file in Directory.GetFiles(sourceDir))
        {
            ...
            Vysora.Logger.Info($"Copied file: {fileName}");
        }
        ...
        foreach (string directory in Directory.GetDirectories(sourceDir))
        {
            ...
            if (!Directory.Exists(destDir))
            {
                ...
                Vysora.Logger.Info($"Created directory: {destDir}");
            }
    ...
    private void ConfigureGameObjects()
    {
        try
        {
            ...
            Vysora.Logger.Info("Starting configuration of game objects with scripts and assets");
            ...
            if (allSuccessful)
            {
                ...
                Vysora.Logger.Info("Game object configuration completed successfully");
            ...
            else
            {
                ...
                EditorUtility.DisplayDialog("Configuration Warning",
                    ...
                    "OK");
                Vysora.Logger.Warning($"Game object configuration completed with errors: {errorLog.ToString()}");
        ...
        {
            Vysora.Logger.Exception(ex, "Configure game objects");
    ...
    private bool ConfigureUIDocument()
    {
        try
        {
            Vysora.Logger.Info("Configuring UI Document with UI.uxml");

            // Find the UI Document GameObject
            GameObject uiDocumentObj = GameObject.Find("UI Document");
            if (uiDocumentObj == null)
            {
                Vysora.Logger.Warning("UI Document GameObject not found in scene. Creating one.");
            ...
            if (uiDocument == null)
            {
                Vysora.Logger.Warning("UIDocument component not found. Adding one.");
            ...
            if (uiDocument.panelSettings == null)
            {
                ...
                Vysora.Logger.Info("Assigned PanelSettings to UI Document");
            }
            ...
            if (uiAsset == null)
            {
                ...
                Vysora.Logger.Warning($"UI.uxml not found at {uiAssetPath}, searching for it...");
                ...
                if (foundFiles.Length > 0)
                {
                    ...
                    Vysora.Logger.Info($"Found UI.uxml at: {relativePath}");
                    uiAsset = AssetDatabase.LoadAssetAtPath<VisualTreeAsset>(relativePath);

                    if (uiAsset == null)
                    {
                        Vysora.Logger.Error($"Found UI.uxml file at {relativePath} but couldn't load it as VisualTreeAsset");
                        return false;
                    }
                }
                else
                {
                    Vysora.Logger.Error("Could not find UI.uxml anywhere in the UI folder");
                    return false;
                }
            }
            ...
            Vysora.Logger.Info($"Successfully configured UI Document with UI.uxml from {AssetDatabase.GetAssetPath(uiAsset)}");
        ...
        {
            Vysora.Logger.Exception(ex, "Configure UI Document");
            return false;
        }
    }
    ...
    private bool ConfigureMainCamera()
    {
        try
        {
            Vysora.Logger.Info("Configuring Main Camera with SimpleCameraController");

            // Find the Main Camera
            GameObject mainCamera = GameObject.FindWithTag("MainCamera");
            if (mainCamera == null)
            {
                Vysora.Logger.Warning("Main Camera not found in scene. Creating one.");
            ...
            if (cameraScript == null)
            {
                ...
                Vysora.Logger.Warning($"SimpleCameraController script not found at {scriptPath}, searching for it...");
                ...
                if (guids.Length > 0)
                {
                    ...
                    Vysora.Logger.Info($"Found SimpleCameraController script at: {foundPath}");
                    cameraScript = AssetDatabase.LoadAssetAtPath<MonoScript>(foundPath);
                }

                if (cameraScript == null)
                {
                    Vysora.Logger.Error("Could not find SimpleCameraController script anywhere in the project");
                    return false;
                }
            }
            ...
            if (mainCamera.GetComponent(cameraScript.GetClass()) == null)
            {
                ...
                Vysora.Logger.Info("Added SimpleCameraController to Main Camera");
            }
            else
            {
                Vysora.Logger.Info("Main Camera already has SimpleCameraController, skipping");
            }
        ...
        {
            Vysora.Logger.Exception(ex, "Configure Main Camera");
            return false;
        }
    }
    ...
    private bool ConfigureUIDocumentScripts()
    {
        try
        {
            Vysora.Logger.Info("Configuring UI Document GameObject with scripts");

            // Find the UI Document GameObject
            GameObject uiDocumentObj = GameObject.Find("UI Document");
            if (uiDocumentObj == null)
            {
                Vysora.Logger.Error("UI Document GameObject not found");
                return false;
            }
            ...
            Dictionary<string, string> scripts = new Dictionary<string, string>()
        {
           // { "MenuNavigationManager", "Assets/Scripts/UI Scripts/MenuNavigationManager.cs" },
            ...
            foreach (var script in scripts)
            {
                ...
                if (monoScript == null)
                {
                    Vysora.Logger.Error($"{scriptName} script not found at path: {scriptPath}");
                    continue;
                }
                ...
                if (uiDocumentObj.GetComponent(monoScript.GetClass()) == null)
                {
                    ...
                    Vysora.Logger.Info($"Added {scriptName} to UI Document GameObject");
                }
                else
                {
                    Vysora.Logger.Info($"UI Document already has {scriptName}, skipping");
                }
            }
        ...
        {
            Vysora.Logger.Exception(ex, "Configure UI Document Scripts");
            return false;
        }
    }
    ...
    private bool ConfigureTileMap()
    {
        try
        {
            Vysora.Logger.Info("Creating and configuring TileMap GameObject");

            // Check if TileMap GameObject already exists
            GameObject tileMapObj = GameObject.Find("TileMap");
            if (tileMapObj == null)
            {
                Vysora.Logger.Info("Creating new TileMap GameObject");
            ...
            if (guids.Length == 0)
            {
                Vysora.Logger.Error("Could not find TileMapGenerator script anywhere in the project");
                return false;
            }
            ...
            {
                Vysora.Logger.Error($"Failed to load TileMapGenerator script from path: {scriptPath}");
                return false;
            }
            ...
            if (tileMapObj.GetComponent(tileMapScript.GetClass()) == null)
            {
                ...
                Vysora.Logger.Info($"Added TileMapGenerator to TileMap GameObject from {scriptPath}");
            }
            else
            {
                Vysora.Logger.Info("TileMap already has TileMapGenerator, skipping");
            }
        ...
        {
            Vysora.Logger.Exception(ex, "Configure TileMap");
            return false;
        }
    }
    #endregion
    ...
    private bool ConfigureMenuNavigationManager()
    {
        try
        {
            Vysora.Logger.Info("Configuring SimpleMenuNavigator");
            ...
            foreach (string scriptName in scriptNames)
            {
                ...
                if (foundScripts.Length > 0)
                {
                    ...
                    if (script != null)
                    {
                        ...
                        Vysora.Logger.Info($"Found {scriptName} script at: {scriptPath}");
                        break;
                    }
                }
            }
            ...
            if (script == null)
            {
                Vysora.Logger.Error("Could not find SimpleMenuNavigator or MenuNavigationManager script");
                return false;
            }
            ...
            if (menuNavigator == null)
            {
                ...
                if (scriptType != null)
                {
                    ...
                    if (menuNavigator == null)
                    {
                        Vysora.Logger.Error($"Failed to add {foundScriptName} component to UI Document");
                        return false;
                    }
                    Vysora.Logger.Info($"Added {foundScriptName} component to UI Document");
                }
                else
                {
                    Vysora.Logger.Error($"Could not get script class for {foundScriptName}");
                    return false;
                }
            }
            else
            {
                Vysora.Logger.Info($"{foundScriptName} already exists on UI Document, configuring properties");
            }
            ...
            if (menuNavigator != null)
            {
                ...
                if (uiDocument == null)
                {
                    Vysora.Logger.Error("UIDocument component not found on UI Document GameObject");
                    return false;
                }
                ...
                foreach (var fieldName in new[] { "mainUIDocument", "uiDocument", "document", "uiDoc" })
                {
                    ...
                    if (field != null && field.FieldType.IsAssignableFrom(typeof(UIDocument)))
                    {
                        ...
                        Vysora.Logger.Info($"Set {fieldName} to UIDocument reference");
                        setUIDoc = true;
                        break;
                    }
                }
                ...
                if (!setUIDoc)
                {
                    Vysora.Logger.Warning("Could not find a field to set the UIDocument reference");
                }
                ...
                if (!string.IsNullOrEmpty(scriptPath) && File.Exists(scriptPath))
                {
                    ...
                    Vysora.Logger.Info("Successfully configured SimpleMenuNavigator properties using SerializedObject");
                    return true;
                }
                else
                {
                    Vysora.Logger.Warning($"Could not find script file at {scriptPath}");
                }
            }
        ...
        {
            Vysora.Logger.Exception(ex, "Configure SimpleMenuNavigator");
            return false;
        }
    ...
    private void SetPropertyValue(Type type, object obj, string propertyName, object value)
    {
        try
        {
            ...
            if (field != null)
            {
                ...
                Vysora.Logger.Info($"Set field {propertyName} = {value}");
                return;
            }
            ...
            if (property != null)
            {
                ...
                Vysora.Logger.Info($"Set property {propertyName} = {value}");
                return;
            }

            Vysora.Logger.Warning($"Could not find field or property {propertyName} on {type.Name}");
        }
        catch (Exception ex)
        {
            Vysora.Logger.Exception(ex, $"Set {propertyName}");
        }
    }
    ...
    private Type FindTypeInAssembly(Assembly assembly, string typeName)
    {
        ...
        Vysora.Logger.Warning($"Could not find type {typeName} in assembly {assembly.GetName().Name}");
        return null;
    }
    ...
    private bool TrySetFieldValue(Type type, object obj, string[] possibleFieldNames, object value)
    {
        foreach (string fieldName in possibleFieldNames)
        {
            ...
            {
                field.SetValue(obj, value);
                Vysora.Logger.Info($"Set {fieldName} = {value}");
                return true;
            }
        }

        // If we get here, none of the field names matched
        Vysora.Logger.Warning($"Could not find any matching field among: {string.Join(", ", possibleFieldNames)}");
        return false;
    }
    ...
    private void SetFieldValue(Type type, object obj, string fieldName, object value)
    {
        ...
        {
            Vysora.Logger.Warning($"Field {fieldName} not found on {type.Name}");
        }
    }
    ...
    private bool ConfigureSimpleCameraController()
    {
        try
        {
            Vysora.Logger.Info("Configuring SimpleCameraController properties");

            // Find the Main Camera with SimpleCameraController
            GameObject mainCamera = GameObject.FindWithTag("MainCamera");
            if (mainCamera == null)
            {
                Vysora.Logger.Error("Main Camera not found in scene");
                return false;
            }
            ...
            {
                Vysora.Logger.Error("SimpleCameraController component not found on Main Camera");
                return false;
            }
            ...
            if (pivotPoint == null)
            {
                ...
                Vysora.Logger.Info("Created camera pivot point cube");
            }
            ...
            if (uiDocument == null)
            {
                Vysora.Logger.Warning("UI Document not found. Some UI integration features may not work.");
            }
            ...
            Vysora.Logger.Info("Successfully configured SimpleCameraController properties");
            return true;
        }
        catch (Exception ex)
        {
            Vysora.Logger.Exception(ex, "Configure SimpleCameraController");
            return false;
        }
    }
    ...
    private bool SetupBackdropModel()
    {
        try
        {
            Vysora.Logger.Info("Setting up backdrop model");

            // Check if the backdrop already exists
            GameObject existingBackdrop = GameObject.Find("BackDrop");
            if (existingBackdrop != null)
            {
                Vysora.Logger.Info("BackDrop object already exists, updating properties");
                ...
                {
                    Vysora.Logger.Warning("Could not find Mtl_Env material for backdrop");
                }

                // Log scale values to check if they're being applied
                Vysora.Logger.Info($"Backdrop scale set to: {existingBackdrop.transform.localScale}");
            ...
            if (cylinderPrefab == null)
            {
                ...
                if (foundAssets.Length > 0)
                {
                    ...
                    Vysora.Logger.Info($"Found Cylinder2 model at: {foundPath}");
                }

                if (cylinderPrefab == null)
                {
                    Vysora.Logger.Error("Could not find Cylinder2 model in 3DModel/Environment folder");
                    return false;
                }
            }
            ...
            if (newMaterial != null)
            {
                ...
                Vysora.Logger.Info("Assigned Mtl_Env material to backdrop");
            }
            else
            {
                ...
                if (foundMaterials.Length > 0)
                {
                    ...
                    if (searchedMaterial != null)
                    {
                        ...
                        Vysora.Logger.Info($"Assigned Mtl_Env material from: {foundPath}");
                    }
                    else
                    {
                        Vysora.Logger.Warning("Could not find Mtl_Env material for backdrop");
            ...
            // Log scale values to check if they're being applied
            Vysora.Logger.Info($"New backdrop scale set to: {backdropObj.transform.localScale}");

            Vysora.Logger.Info("Successfully set up backdrop model");
            return true;
        }
        catch (Exception ex)
        {
            Vysora.Logger.Exception(ex, "Setup backdrop model");
            return false;
        }
    }
    ...
    private bool ConfigureButtonLabelController()
    {
        try
        {
            Vysora.Logger.Info("Configuring ButtonLabelController properties");
            ...
            {
                Vysora.Logger.Error("ButtonLabelController component not found on UI Document");
                return false;
            }
            ...
            Vysora.Logger.Info("Successfully configured ButtonLabelController properties");
            return true;
        }
        catch (Exception ex)
        {
            Vysora.Logger.Exception(ex, "Configure ButtonLabelController");
            return false;
        }
    }
    ...
    private bool ConfigureEnhancedFullScreenToggle()
    {
        try
        {
            Vysora.Logger.Info("Configuring EnhancedFullScreenToggle properties");
            ...
            {
                Vysora.Logger.Error("EnhancedFullScreenToggle component not found on UI Document");
                return false;
            }
            ...
            Vysora.Logger.Info("Successfully configured EnhancedFullScreenToggle properties");
            return true;
        }
        catch (Exception ex)
        {
            Vysora.Logger.Exception(ex, "Configure EnhancedFullScreenToggle");
            return false;
        }
    }
    ...
    private bool ConfigureEnhancedScreenshotManager()
    {
        try
        {
            Vysora.Logger.Info("Configuring EnhancedScreenshotManager properties");
            ...
            {
                Vysora.Logger.Error("EnhancedScreenshotManager component not found on UI Document");
                return false;
            }
            ...
            Vysora.Logger.Info("Successfully configured EnhancedScreenshotManager properties");
            return true;
        }
        catch (Exception ex)
        {
            Vysora.Logger.Exception(ex, "Configure EnhancedScreenshotManager");
            return false;
        }
    }
    ...
    private bool ConfigurePopupManager()
    {
        try
        {
            Vysora.Logger.Info("Configuring PopupManager properties");
            ...
            {
                Vysora.Logger.Error("PopupManager component not found on UI Document");
                return false;
            }
            ...
            Vysora.Logger.Info("Successfully configured PopupManager properties");
            return true;
        }
        catch (Exception ex)
        {
            Vysora.Logger.Exception(ex, "Configure PopupManager");
            return false;
        }
    }
    ...
    private VisualTreeAsset FindVisualTreeAsset(string name)
    {
        ...
        Vysora.Logger.Warning($"VisualTreeAsset '{name}' not found. This might need to be assigned manually.");
        return null;
    }
    ...
    private Type FindType(string typeName)
    {
        ...
        Vysora.Logger.Warning($"Type '{typeName}' not found. Using Object type as fallback.");
    ...
    private object CreateMenuConfigObject(Type containerType, string menuName, VisualTreeAsset menuTemplate)
    {
        ...
        if (menuConfigType == null)
        {
            Vysora.Logger.Warning("MenuConfig type not found. Using Dictionary as fallback.");
    ...
    private object CreateButtonMappingObject(string buttonName, VisualTreeAsset targetMenu, string menuHeaderText)
    {
        ...
        {
            Vysora.Logger.Warning("ButtonMapping type not found. Using Dictionary as fallback.");
            var dict = new Dictionary<string, object>();
    ...
    private bool ConfigureScriptProperties()
    {
        try
        {
            ...
            Vysora.Logger.Info("Starting configuration of script properties");
            ...
            if (!step9Success)
            {
                ...
                Vysora.Logger.Info("User chose not to switch platform to WebGL");
            }
            downloadProgress = 0.9f;
            ...
            if (allSuccessful)
            {
                ...
                Vysora.Logger.Info("Script properties configuration completed successfully");
            ...
            else
            {
                ...
                EditorUtility.DisplayDialog("Configuration Warning",
                    $"Some configuration steps failed:\n{errorLog.ToString()}\n\nYou may need to manually configure these components.",
                    "OK");
                Vysora.Logger.Warning($"Script properties configuration completed with errors: {errorLog.ToString()}");
        ...
        {
            Vysora.Logger.Exception(ex, "Configure script properties");
            EditorUtility.DisplayDialog("Configuration Error",
    ...
    private bool ConfigureEnvironmentLighting()
    {
        try
        {
            Vysora.Logger.Info("Configuring environment lighting");

            // Set the ambient light to Color mode and #777A7F
            RenderSettings.ambientMode = UnityEngine.Rendering.AmbientMode.Flat;
            RenderSettings.ambientLight = HexToColor("#777A7F");

            Vysora.Logger.Info("Successfully configured environment lighting");
            return true;
        }
        catch (Exception ex)
        {
            Vysora.Logger.Exception(ex, "Configure environment lighting");
            return false;
        }
    }
    ...
    private bool ConfigureURPAsset()
    {
        try
        {
            Vysora.Logger.Info("Setting up URP Asset");
            ...
            if (existingUrpAsset == null || existingRendererAsset == null)
            {
                ...
                if (vysoraUrpAsset != null && vysoraRendererAsset != null)
                {
                    ...
                    AssetDatabase.Refresh();
                    Vysora.Logger.Info("Copied URP and Renderer assets from Vysora folder to Assets root");
                }
                else
                {
            ...
            if (needsDownload)
            {
                Vysora.Logger.Info("URP assets not found in project, downloading from repository");
                ...
                if (existingUrpAsset == null)
                {
                    ...
                    if (!urpDownloaded)
                    {
                        Vysora.Logger.Error("Failed to download URP Asset");
                        return false;
                    }
                }
                ...
                if (existingRendererAsset == null)
                {
                    ...
                    {
                        Vysora.Logger.Error("Failed to download URP Renderer Asset");
                        return false;
                    }
                }
            ...
            if (urpAsset == null)
            {
                Vysora.Logger.Error($"Failed to load URP Asset at {urpAssetDestPath}");
                return false;
            }

            if (rendererAsset == null)
            {
                Vysora.Logger.Error($"Failed to load URP Renderer Asset at {rendererAssetDestPath}");
                return false;
            }
            ...
            Vysora.Logger.Info("Set URP Asset in Graphics settings");

            // Set the URP Asset in Quality settings
            QualitySettings.renderPipeline = urpAsset;
            Vysora.Logger.Info("Set URP Asset in Quality settings");
            ...
            Vysora.Logger.Info("Successfully configured URP Asset");
            return true;
        }
        catch (Exception ex)
        {
            Vysora.Logger.Exception(ex, "Configure URP Asset");
            return false;
        }
    ...
    private void ConfigureURPShadowSettings(UnityEngine.Rendering.RenderPipelineAsset renderPipelineAsset)
    {
        try
        {
            Vysora.Logger.Info("Configuring URP shadow settings for shader compatibility");
            ...
            Vysora.Logger.Info("Available URP asset properties:");
            while (iterator.NextVisible(enterChildren))
            {
                enterChildren = false;
                Vysora.Logger.Info($"- Property: {iterator.propertyPath}, Type: {iterator.propertyType}");
            }
            ...
            while (iterator.NextVisible(true))
            {
                    ...
                    propertyPath.Contains("Support") && iterator.propertyType == SerializedPropertyType.Boolean)
                {
                    ...
                    Vysora.Logger.Info($"Enabled main light shadows using property: {propertyPath}");
                ...
                {
                    iterator.boolValue = true;
                    Vysora.Logger.Info($"Enabled soft shadows using property: {propertyPath}");
                    softShadowsSet = true;
                }
                ...
                if (propertyPath.EndsWith("shadowDistance") || propertyPath.EndsWith("ShadowDistance"))
                {
                    ...
                    Vysora.Logger.Info($"Set shadow distance to 50 using property: {propertyPath}");
                ...
                {
                    iterator.intValue = 4;
                    Vysora.Logger.Info($"Set shadow cascade count to 4 using property: {propertyPath}");
            ...
            if (!mainLightShadowsSet)
                Vysora.Logger.Warning("Could not find main light shadows property in URP asset");
            if (!softShadowsSet)
                Vysora.Logger.Warning("Could not find soft shadows property in URP asset");
            if (!shadowDistanceSet)
                Vysora.Logger.Warning("Could not find shadow distance property in URP asset");
            if (!cascadeCountSet)
                Vysora.Logger.Warning("Could not find cascade count property in URP asset");
            ...
            Vysora.Logger.Info("URP shadow settings configured for shader compatibility");
        }
        catch (Exception ex)
        {
            Vysora.Logger.Exception(ex, "Configure URP Shadow Settings");
        }
    }
    ...
    private void ForceShaderRecompilation(string shaderName)
    {
        try
        {
            Vysora.Logger.Info($"Looking for shader with name: {shaderName}");
            ...
            {
                Vysora.Logger.Info($"Checking path: {path}");
                shader = AssetDatabase.LoadAssetAtPath<Shader>(path);
                if (shader != null)
                {
                    Vysora.Logger.Info($"Found shader at path: {path}");
                    break;
                }
            }
            ...
            if (shader == null)
            {
                Vysora.Logger.Info("Searching for shader by name in all project assets...");
                ...
                foreach (string guid in guids)
                {
                        ...
                        (potentialShader.name.Contains(basicName) || assetPath.Contains(basicName)))
                    {
                        ...
                        Vysora.Logger.Info($"Found shader by name search: {potentialShader.name} at {assetPath}");
                        break;
                    }
                }
            }
            ...
            if (shader == null)
            {
                Vysora.Logger.Info("Searching for shader in all loaded resources...");
                string basicName = Path.GetFileNameWithoutExtension(shaderName);
                ...
                foreach (Shader s in allShaders)
                {
                    ...
                    {
                        shader = s;
                        Vysora.Logger.Info($"Found shader in resources: {s.name}");
                        break;
                    }
                }
            }
            ...
            if (shader != null)
            {
                Vysora.Logger.Info($"Forcing recompilation of shader: {shader.name}");
                ...
                {
                    Vysora.Logger.Warning($"Some properties might not exist on the material: {propEx.Message}");
                }
                ...
                Vysora.Logger.Info("Shader recompilation triggered");
            }
            else
            {
                Vysora.Logger.Warning($"Could not find shader with name: {shaderName} in the project");
                ...
                {
                    Vysora.Logger.Info($"Found {backdropShaders.Length} backdrop-related shaders:");
                    foreach (var s in backdropShaders)
                    {
                        Vysora.Logger.Info($"- {s.name}");
                    }
                }
                else
                {
                    Vysora.Logger.Warning("No backdrop-related shaders found in the project");
                }
            }
        }
        catch (Exception ex)
        {
            Vysora.Logger.Exception(ex, "Force Shader Recompilation");
        }
    ...
    private bool DownloadAssetFromRepo(string fileName, string destPath, string assetType)
    {
        try
        {
            Vysora.Logger.Info($"Downloading {assetType} from GitHub");
            ...
            using (HttpClient client = new HttpClient())
            {
                ...
                if (isPrivateRepo)
                {
                    ...
                    {
                        Vysora.Logger.Error("Authentication required for private repo but no credentials provided");
                        EditorUtility.ClearProgressBar();
                        return false;
                    }
                }
                ...
                {
                    Vysora.Logger.Error($"Failed to download {assetType}: HTTP {(int)response.StatusCode} - {response.ReasonPhrase}");
                    EditorUtility.ClearProgressBar();
                    return false;
                }
                ...
                Vysora.Logger.Info($"{assetType} downloaded to {destPath}");
                EditorUtility.ClearProgressBar();
                return true;
            }
        }
        ...
        {
            Vysora.Logger.Exception(ex, $"Download {assetType}");
            EditorUtility.ClearProgressBar();
            return false;
        }
    }
    ...
    private void AssignRendererToURPAsset(UnityEngine.Rendering.RenderPipelineAsset urpAsset, ScriptableObject rendererAsset)
    {
        try
        {
            ...
            if (rendererList != null)
            {
                ...
                serializedURPAsset.ApplyModifiedProperties();
                Vysora.Logger.Info("Assigned renderer to URP asset");
            }
            else
            {
                Vysora.Logger.Warning("Could not find renderer list property on URP asset");
            }
        }
        catch (Exception ex)
        {
            Vysora.Logger.Exception(ex, "Assign Renderer To URP Asset");
        }
    }
    ...
    private bool DownloadURPAsset(string destPath)
    {
        try
        {
            Vysora.Logger.Info("Downloading URP Asset from GitHub");
            ...
            using (HttpClient client = new HttpClient())
            {
                ...
                if (!response.IsSuccessStatusCode)
                {
                    Vysora.Logger.Error($"Failed to download URP Asset: HTTP {(int)response.StatusCode} - {response.ReasonPhrase}");
                    EditorUtility.ClearProgressBar();
                    return false;
                }
                ...
                Vysora.Logger.Info($"URP Asset downloaded to {destPath}");
                EditorUtility.ClearProgressBar();
                return true;
            }
        }
        ...
        {
            Vysora.Logger.Exception(ex, "Download URP Asset");
            EditorUtility.ClearProgressBar();
            return false;
        }
    }
    ...
    private bool PromptForPlatformSwitch()
    {
        try
        {
            Vysora.Logger.Info("Prompting for platform switch to WebGL");
            ...
            if (switchToWebGL)
            {
                Vysora.Logger.Info("User chose to switch to WebGL platform");
                ...
                {
                    Vysora.Logger.Info("Project is already using WebGL platform");
                    return true;
                }
                ...
                if (!BuildPipeline.IsBuildTargetSupported(targetGroup, target))
                {
                    ...
                    Vysora.Logger.Warning("WebGL module is not installed");
                    return false;
                }
            ...
            {
                Vysora.Logger.Info("User chose not to switch to WebGL platform");
                return false;
            }
        }
        catch (Exception ex)
        {
            Vysora.Logger.Exception(ex, "Prompt for platform switch");
            return false;
        }
    }
    ...
    private bool SetupBackdropWithShadowReceiver()
    {
        try
        {
            Vysora.Logger.Info("Setting up BackDrop with SolidColorShadowReceiver shader");
            ...
            {
                Vysora.Logger.Info("SolidColorShadowReceiver shader not found, downloading from repository");
                ...
                if (!shaderDownloaded)
                {
                    Vysora.Logger.Error("Failed to download SolidColorShadowReceiver shader");
                    return false;
                }
                ...
                if (existingShader == null)
                {
                    Vysora.Logger.Error("Failed to load downloaded shader");
                    return false;
                }
            }
            ...
            if (backdrop != null)
            {
                ...
                if (renderer != null)
                {
                    ...
                    Vysora.Logger.Info("Successfully assigned SolidColorShadowReceiver material to BackDrop");
                ...
                {
                    Vysora.Logger.Warning("BackDrop object found but it has no Renderer component");
                }
            }
            else
            {
                Vysora.Logger.Warning("Could not find BackDrop object in the scene");
                ...
                {
                    Vysora.Logger.Info($"Found {possibleBackdrops.Length} possible backdrop objects:");
                    foreach (var obj in possibleBackdrops)
                    {
                        Vysora.Logger.Info($"- {obj.name}");
                        ...
                        {
                            renderer.sharedMaterial = shadowReceiverMaterial;
                            Vysora.Logger.Info($"Assigned SolidColorShadowReceiver material to {obj.name}");
                ...
                {
                    Vysora.Logger.Warning("No objects with backdrop-like names found in the scene");
                }
            }
        ...
        {
            Vysora.Logger.Exception(ex, "Setup BackDrop with Shadow Receiver");
            return false;
        }
    }
